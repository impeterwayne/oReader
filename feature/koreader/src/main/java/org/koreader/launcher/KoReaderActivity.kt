...
import com.genesys.feature.koreader.R
import org.koreader.launcher.device.Device
import org.koreader.launcher.dialog.LightDialog
import org.koreader.launcher.extensions.aardAction
import org.koreader.launcher.extensions.absolutePath
import org.koreader.launcher.extensions.colordictAction
import org.koreader.launcher.extensions.filePicker
import org.koreader.launcher.extensions.getAvailableHeight
import org.koreader.launcher.extensions.getAvailableWidth
import org.koreader.launcher.extensions.getBarHeight
import org.koreader.launcher.extensions.getHeight
import org.koreader.launcher.extensions.getOrientationCompat
import org.koreader.launcher.extensions.getSdcardPath
import org.koreader.launcher.extensions.getWidth
import org.koreader.launcher.extensions.hapticFeedback
import org.koreader.launcher.extensions.networkInfo
import org.koreader.launcher.extensions.openWifi
import org.koreader.launcher.extensions.platform
import org.koreader.launcher.extensions.processTextAction
import org.koreader.launcher.extensions.quickdicAction
import org.koreader.launcher.extensions.requestSpecialPermission
import org.koreader.launcher.extensions.searchAction
import org.koreader.launcher.extensions.sendAction
import org.koreader.launcher.extensions.setOrientationCompat
import org.koreader.launcher.extensions.toFile
import timber.log.Timber
import java.io.File
import java.util.Locale
import java.util.concurrent.CountDownLatch

class KoReaderActivity : NativeActivity(), LuaInterface,
    ...
    companion object {
        ...
        const val EXTRA_OPEN_FILE_PATH = "com.genesys.feature.koreader.extra.OPEN_FILE_PATH"

        fun fileManagerIntent(context: Context): Intent {
            return Intent(context, KoReaderActivity::class.java)
        }
        ...
        fun openDocumentIntent(context: Context, filePath: String): Intent {
            return Intent(context, KoReaderActivity::class.java).apply {
                putExtra(EXTRA_OPEN_FILE_PATH, filePath)
            }
        }
    ...
    init {
        Timber.tag(tag).i("loading libluajit-launcher")
        System.loadLibrary("luajit-launcher")
        Timber.tag(tag).i("loading libluajit")
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        MainApp.initialize(applicationContext)
        assets = Assets()
        device = Device(this)
        timeout = Timeout()
        event = EventReceiver()
        ...
        Timber.tag(TAG_SURFACE).v("Using $surfaceKind implementation")

        @Suppress("DEPRECATION")
        screenIsLandscape = windowManager.defaultDisplay.width > windowManager.defaultDisplay.height

        Timber.tag(tag).v(String.format(Locale.US,
    ...
    override fun surfaceChanged(holder: SurfaceHolder, format: Int, width: Int, height: Int) {
        Timber.tag(TAG_SURFACE).v(String.format(Locale.US,
            "surface changed {\n  width:  %d\n  height: %d\n  format: %s\n}",
    ...
    override fun onAttachedToWindow() {
        Timber.tag(TAG_SURFACE).d("onAttachedToWindow()")
    ...
    override fun onNewIntent(intent: Intent) {
        ...
        Timber.tag(tag).d("onNewIntent(): $scheme")
        ...
        if (requestCode == MANDATORY_PERMISSIONS_ID) {
            for (i in permissions.indices) {
                if (grantResults[i] != PackageManager.PERMISSION_GRANTED) {
                    Timber.tag(tag).e("mandatory permission rejected: ${permissions[i]}. Bye!")
        ...
        text?.let { lookupText ->
            action?.let { lookupAction ->
                when (lookupAction) {
                    ...
                    else -> Timber.tag(tag).w("Unsupported action $lookupAction")
                }
            } ?: Timber.tag(tag).e("invalid lookup: no action")
        } ?: Timber.tag(tag).e("invalid lookup: no text")
    }
    ...
    override fun getFilePathFromIntent(): String? {
        return intent?.let {
            Timber.tag(tag).i("New intent: $it.action")
            val explicitPath = it.getStringExtra(EXTRA_OPEN_FILE_PATH)
            if (!explicitPath.isNullOrBlank() && File(explicitPath).exists()) {
                Timber.tag(tag).i("Found explicit file path -> $explicitPath")
                explicitPath
        ...
        if (splashScreen) {
            ...
            holder.lockCanvas()?.let { canvas ->
                try {
                    ...
                    Timber.tag(tag).w(e, "Failed to draw splash screen")
                }
    ...
    private fun registerEventReceiver() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            // Download completion broadcasts are delivered by the system download provider,
            // so the receiver must be exported on Android 13+.
            registerReceiver(event, event.filter, RECEIVER_EXPORTED)
        } else {
            @Suppress("DEPRECATION")

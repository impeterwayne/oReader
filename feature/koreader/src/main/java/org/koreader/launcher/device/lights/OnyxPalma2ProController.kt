...
class OnyxPalma2ProController : LightsInterface {
    ...
    private val cachedBrightnessMax: Int by lazy {
        try {
            ...
            Timber.tag(TAG).w( "Failed to read max brightness, using default: $e")
    ...
    private val cachedWarmthMax: Int by lazy {
        try {
            ...
            Timber.tag(TAG).w( "Failed to read max warmth, using default: $e")
            DEFAULT_WARMTH_MAX
        }
    }
    ...
    override fun getBrightness(activity: Activity): Int {
        return try {
            ...
            Timber.tag(TAG).w( "Failed to read brightness: $e")
            0
        }
    }
    ...
    override fun getWarmth(activity: Activity): Int {
        return try {
            ...
            Timber.tag(TAG).w( "Failed to read warmth: $e")
            0
        }
    }
    ...
    override fun setBrightness(activity: Activity, brightness: Int) {
        if (brightness !in MIN..cachedBrightnessMax) {
            Timber.tag(TAG).w( "brightness value out of range: $brightness (max: $cachedBrightnessMax)")
            return
        }
        ...
        try {
            ...
            Timber.tag(TAG).e( "Failed to set brightness: $e")
        }
    }

    override fun setWarmth(activity: Activity, warmth: Int) {
        if (warmth !in MIN..cachedWarmthMax) {
            Timber.tag(TAG).w( "warmth value out of range: $warmth (max: $cachedWarmthMax)")
            return
        }
        ...
        try {
            ...
            Timber.tag(TAG).e( "Failed to set warmth: $e")
        }
    }

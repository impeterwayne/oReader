...
class Timeout {
    ...
    fun setTimeout(activity: Activity, ms: Int) {
        when {
            ...
            ms > SCREEN_ON_DISABLED -> {
                ...
                Timber.tag(tag).v("applying activity custom timeout: $mins minutes")
    ...
    */
    ...
    private fun apply(activity: Activity, resumed: Boolean) {
        ...
        if (resumed && customTimeout) {
            if (appTimeout > 0) {
                ...
                Timber.tag(tag).w("$logMsg: custom timeout is 0, ignoring")
            }
            ...
            if (systemTimeout > 0) {
                ...
                Timber.tag(tag).w("$logMsg: system timeout is 0, ignoring")
            }
        } else {
            Timber.tag(tag).v(logMsg)
        }
    }
    ...
    private fun setScreenOn(activity: Activity, enable: Boolean) {
        if (enable != alwaysOn) {
            Timber.tag(tag).v("screen on: switching to $enable")
            alwaysOn = enable
            val flag = WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON
            val action = if (enable) "add" else "clear"
            activity.runOnUiThread {
                Timber.tag(tag).v("$action FLAG_KEEP_SCREEN_ON")

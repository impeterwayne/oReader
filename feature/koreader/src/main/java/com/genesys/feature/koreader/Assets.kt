...
import com.genesys.feature.koreader.extensions.pruneCacheDir
...
class Assets {
    ...
    init {
        Timber.tag(tag).i("loading ${BuildConfig.SEVENZIP_LIB}")
    ...
    fun extract(activity: Activity): Boolean {
        return if (isNewBundle(activity)) {
            ...
            Timber.tag(tag).i("update installed in ${elapsedTime / 1000000} milliseconds")
        ...
        return try {
            ...
            context.assets.open("module/version.txt").bufferedReader().use {
                ...
                return if (version == installedVersion) {
                    Timber.tag(tag).i("Skip installation for revision $version")
                    false
                } else {
                    Timber.tag(tag).i("Found new package revision $version")
                    true
                }
            }
        } catch (e: Exception) {
    ...
    private fun bootstrap(activity: Activity): Boolean {
        ...
        try {
            ...
            Timber.tag(tag).e(e, "Error prunning old files")
        }
                ...
                if (assetName != "module/version.txt") {
                    when {
                        (asset.endsWith("7z")) -> {
                            ...
                            Timber.tag(tag).v("Uncompressing $assetName")
                            try {
                                val ok = (extract(activity.assets, assetName, filesDir) == 0)
                                if (!ok)
                                    return false
                            } catch (e: Exception) {
                                Timber.tag(tag).e(e, "Error extracting 7z file")
                                return false
                            }
                        }
                        else -> {
                            /* Copy all regular files in assets store */
                            Timber.tag(tag).v("Extracting $assetName")
                            try {

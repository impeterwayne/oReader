...
class GetNotebookImageByIdUseCase @Inject constructor(
    private val notebookImageRepository: NotebookImageRepository
) {
    suspend operator fun invoke(imageId: String): NotebookImage {
        return notebookImageRepository.getById(imageId)
    }
}

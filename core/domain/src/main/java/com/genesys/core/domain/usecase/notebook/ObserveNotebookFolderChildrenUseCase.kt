...
class ObserveNotebookFolderChildrenUseCase @Inject constructor(
    private val notebookFolderRepository: NotebookFolderRepository
) {
    operator fun invoke(folderId: String? = null): Flow<List<NotebookFolder>> {
        return notebookFolderRepository.observeChildren(folderId)
    }
}

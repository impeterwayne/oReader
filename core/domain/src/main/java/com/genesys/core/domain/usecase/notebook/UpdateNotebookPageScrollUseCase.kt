...
class UpdateNotebookPageScrollUseCase @Inject constructor(
    private val notebookPageRepository: NotebookPageRepository
) {
    suspend operator fun invoke(pageId: String, scroll: Int): Unit {
        return notebookPageRepository.updateScroll(pageId, scroll)
    }
}

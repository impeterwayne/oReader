...
@Singleton
class ReaderSettingsManager @Inject constructor(
    ...
    override suspend fun save(settings: ReaderSettings) {
        val normalized = settings.copy(scanFolders = normalizeFolders(settings.scanFolders))
        _current.value = normalized
        withContext(Dispatchers.IO) {
            try {
                kvRepository.set(NotebookKeyValue(ReaderSettings.KV_KEY, adapter.toJson(normalized)))
            } catch (error: Exception) {
                Timber.e(error, "Failed to persist ReaderSettings")
            }
        }
    }
    ...
    private fun parseSettings(json: String): ReaderSettings {
        return try {
            ...
            Timber.e(error, "Failed to deserialise ReaderSettings")
            ReaderSettings()
        }
    }
